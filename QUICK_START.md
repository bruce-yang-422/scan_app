...
- 第一行必須是標題行
- **日期限制**：系統會自動過濾超過7天的訂單資料，不會匯入過時的訂單
...
1. **選擇 Batch 或總出貨模式**
   ...
   - **非清單內出貨紀錄模式**（僅總出貨模式）：
     - 在總出貨掃描頁面右上角，點擊開關按鈕啟用或關閉
     - 啟用後，如果掃描的條碼不在出貨清單內，且符合蝦皮物流格式（15字元，開頭"TW"），會自動記錄
...
2. **進入掃描頁面**
   ...
   - **總出貨模式統計資訊**：
     - 顯示統計資訊：總筆數、待掃描、已掃描
     - 當開啟非清單內出貨紀錄模式時，會額外顯示「非清單」統計
     - 關閉非清單內出貨紀錄模式時，統計UI自動恢復為三個項目並重新整理
...
5. **選擇操作**
   - 匯出成功後會顯示簡潔的對話框，包含簡化的提示資訊
   - **複製 LINE 版本**：複製簡潔格式到剪貼簿，適合直接貼到 LINE（寬度20字符，只顯示統計摘要，不顯示任何明細）
   - **複製 TXT 內容**：複製完整格式到剪貼簿（包含所有明細）
   - **分享 TXT**：分享 TXT 檔案（文件會正確附加到 LINE，不會顯示為文本）
   - **分享 JSON**：分享 JSON 檔案（用於程式處理）
   - 透過分享選單選擇目標應用：
     - **Line**：分享到 Line 聊天（文件會正確附加）
...
1. **進入設定頁面**
   - 在 Batch 列表頁面，點擊右上角「設定」按鈕（顯示「設定」文字和圖示）
   - 會顯示半頁設定面板
...
8. **資料管理**
   ...
   - **非清單內出貨紀錄模式**：
     - 預設開啟：設定進入總出貨模式時，非清單內出貨紀錄模式的預設開關狀態（預設為關閉）
   - 可清空所有資料（批次、掃描記錄、匯出檔案）

**設定說明**：
- 所有設定會立即生效並自動儲存（**無需重啟 App**）
- **顯示模式**：更改後立即生效，無需重啟應用程式
- **匯入後自動刷新**：匯入或更新批次後，批次列表會自動刷新，無需手動下拉更新
- 系統層使用 UTC 時間儲存和處理（標準時間）
- 匯入匯出時會自動轉換為台灣時區時間（UTC+8）
- 預設時區為 UTC+8 台北
...
- 未完成的 Batch 不會被清理
- **匯出檔案自動清理**：App 啟動時會自動清理超過 10 天的匯出檔案
  - 預設保留天數：10 天
  - 超過保留天數的檔案會自動刪除
  - 空目錄會自動清理
...
- 例如：`TW254833743744Z`、`TW251087895529U`、`TW2557292761802`
- **統計顯示**：
  - 開啟時：統計UI顯示四個項目（總筆數/待掃描/已掃描/非清單），並自動重新整理資料
  - 關閉時：統計UI自動恢復為三個項目（總筆數/待掃描/已掃描）並重新整理資料
