...
- 欄位順序固定：![1766724480414](image/README/1766724480414.png)
...
### 3. 結果匯出與分享
- 匯出為 TXT 格式（人可讀，用於 LINE 傳送）
- 匯出為 JSON 格式（機器可用，留存）
- 匯出檔案包含掃描人員資訊（姓名和ID）
- 可選擇分享 TXT 或 JSON 檔案
- 支援複製 TXT 內容到剪貼簿（可直接貼到 LINE）
- 可重新匯出已完成批次
...
1. **來源名稱**（非必填）：例如「台北一店」、「供應商A」等（可自由設定）
